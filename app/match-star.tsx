...
async function cardImage(match:Match,player:Ranked,performance?:Performance,withoutPhoto=false):Promise<Blob>{
 ...
 if(player.photo_url&&!withoutPhoto){
  ...
  ctx.save();ctx.beginPath();ctx.rect(120,275,840,550);ctx.clip();
  const base=Math.max(840/image.width,550/image.height),zoom=player.photo_zoom??1;
  const w=image.width*base*zoom,h=image.height*base*zoom;
  ctx.drawImage(image,120+(840-w)*(player.photo_x??50)/100,275+(550-h)*player.photo_y/100,w,h);ctx.restore();
 }else{ctx.fillStyle='#f2ce39';ctx.font='bold 200px Arial';ctx.fillText(player.display_name.slice(0,2).toUpperCase(),540,660);}
 const fade=ctx.createLinearGradient(0,580,0,880);fade.addColorStop(0,'#11120e00');fade.addColorStop(1,'#11120e');ctx.fillStyle=fade;ctx.fillRect(120,580,840,310);
 ...
 textFit(player.display_name.toUpperCase(),825,80,gold);
 ctx.strokeStyle='#d6b43266';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(190,855);ctx.lineTo(890,855);ctx.stroke();
 ctx.fillStyle='#fff7d4';ctx.font='bold 64px Arial';ctx.fillText(String(performance?.goals??0),360,925);ctx.fillText(String(performance?.assists??0),720,925);
 ctx.font='22px Arial';ctx.fillStyle='#cfbd82';ctx.fillText('GOLS NA PELADA',360,965);ctx.fillText('ASSISTÊNCIAS NA PELADA',720,965);
 textFit(player.rank+'º NO RANKING  ·  '+player.points+' PTS',1040,42,gold);
 textFit(monthLabel(match.played_on.slice(0,7)).toUpperCase(),1080,23,'#d0c08d');
 textFit(match.name,1150,27,'#f4e5b8');textFit(new Date(match.played_on+'T12:00:00').toLocaleDateString('pt-BR'),1194,24,'#b8aa7e');
 ctx.fillStyle='#e0bf45';ctx.font='22px Arial';ctx.fillText('RANKING DO MÊS · ATUALIZADO EM '+new Date().toLocaleString('pt-BR',{timeZone:'America/Fortaleza',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})+' (PB)',540,1305);
...
export function MatchStar({match,player,performance}:{match:Match;player:Ranked;performance?:Performance}){
 ...
 useEffect(()=>setPreview(null),[player.id,player.rank,player.points,player.photo_url,player.photo_x,player.photo_y,player.photo_zoom,player.display_name,performance?.goals,performance?.assists,match.played_on,match.name]);
 ...
 if(match.status==='cancelled')return null;
 return <section className="match-star" aria-label={'Craque da pelada: '+player.display_name}><div className="star-card"><div className="star-title"><Trophy size={20}/> CRAQUE DA PELADA</div><div className="star-portrait">{player.photo_url?<img src={player.photo_url} alt={player.display_name} style={photoStyle(player)}/>:<span>{player.display_name.slice(0,2).toUpperCase()}</span>}</div><h3>{player.display_name}</h3><div className="star-numbers"><span><b>{performance?.goals??0}</b> GOLS</span><span><b>{performance?.assists??0}</b> ASSIST.</span></div><div className="star-rank"><strong>{player.rank}º no ranking · {player.points} pts</strong><small>{monthLabel(match.played_on.slice(0,7))}</small></div><small>{dateLabel(match.played_on)}</small></div><div className="star-copy"><span className="eyebrow">DESTAQUE DENTRO DE CAMPO</span><h3>O craque foi {player.display_name}.</h3><p>Reconhecimento da pelada. Uma carta para guardar e compartilhar com a turma.</p><div className="star-buttons"><button className="primary" disabled={busy} onClick={()=>exportCard(false)}><Download size={17}/>{busy?'Gerando…':'Baixar carta'}</button><button className="secondary" disabled={busy} onClick={()=>exportCard(true)}><Share2 size={17}/>Compartilhar</button></div><small>Imagem PNG · números da pelada e classificação atual no mês dela</small>{preview&&<details className="star-export" open><summary>Imagem pronta para compartilhar</summary><img src={preview.url} alt="Carta do craque pronta para compartilhar"/><a href={preview.url} download={preview.name}>Salvar imagem PNG</a><small>No celular, você também pode segurar a imagem para salvar.</small></details>}{error&&<div role="alert"><p className="error">{error}</p><button className="secondary" disabled={busy} onClick={()=>exportCard(false,true)}>Baixar sem foto</button></div>}</div></section>;
}
