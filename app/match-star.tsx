...
export function MatchStar({match,player,performance}:{match:Match;player:Ranked;performance?:Performance}){
 ...
 async function exportCard(share:boolean,withoutPhoto=false){setBusy(true);setError('');try{
  ...
  const stamp=clone.querySelector('[data-generated]');if(stamp)stamp.textContent='Gerado em '+new Date().toLocaleString('pt-BR',{timeZone:'America/Fortaleza',day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})+' · PB';
  ...
  let blob:Blob|null;
  try{blob=await toBlob(clone,{pixelRatio:3,skipFonts:true,includeQueryParams:true});}finally{host.remove();}
 ...
 if(match.status==='cancelled')return null;
 return <section className="match-star" aria-label={'Craque da pelada: '+player.display_name}><div className="star-profile-card"><PlayerCard player={player} month={match.played_on.slice(0,7)} large/><div className="star-rank"><Trophy size={16}/><strong>{player.rank}º no ranking · {player.points} pts</strong></div></div><div className="star-copy"><span className="eyebrow">DESTAQUE DENTRO DE CAMPO</span><h3>O craque foi {player.display_name}.</h3><p>Reconhecimento da pelada. Uma carta para guardar e compartilhar com a turma.</p><div className="star-buttons"><button className="primary" disabled={busy} onClick={()=>exportCard(false)}><Download size={17}/>{busy?'Gerando…':'Baixar carta'}</button><button className="secondary" disabled={busy} onClick={()=>exportCard(true)}><Share2 size={17}/>Compartilhar</button></div><small>Imagem PNG · números da pelada e classificação atual no mês dela</small>{preview&&<details className="star-export" open><summary>Imagem pronta para compartilhar</summary><img src={preview.url} alt="Carta do craque pronta para compartilhar"/><a href={preview.url} download={preview.name}>Salvar imagem PNG</a><small>No celular, você também pode segurar a imagem para salvar.</small></details>}{error&&<div role="alert"><p className="error">{error}</p><button className="secondary" disabled={busy} onClick={()=>exportCard(false,true)}>Baixar sem foto</button></div>}</div><div className="star-poster-host" aria-hidden="true"><div className="star-poster" ref={poster}><header><span>P/ PELADA CLUB</span><h2><Trophy size={20}/> CRAQUE DA PELADA</h2></header><PlayerCard player={player} month={match.played_on.slice(0,7)} large/><div className="poster-rank"><strong>{player.rank}º NO RANKING</strong><span>{player.points} PONTOS · {monthLabel(match.played_on.slice(0,7))}</span></div><footer><strong>{match.name}</strong><span>{dateLabel(match.played_on)} · {performance?.goals??0} gols e {performance?.assists??0} assistências na pelada</span><small data-generated>Classificação do mês na geração da imagem</small></footer></div></div></section>;
}
